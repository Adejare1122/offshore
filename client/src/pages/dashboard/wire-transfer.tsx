...
import { type Account, type User } from "@shared/schema";
...
export default function WireTransfer() {
    ...
    const createTransferMutation = useMutation({
        mutationFn: async (data: any) => {
            const res = await apiRequest("POST", "/api/transfers/wire", data);
            return res.json();
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/transactions", `${String(userId ?? "")}`] });
            toast({ title: "Success", description: "Wire transfer initiated. Redirecting…" });
            setTimeout(() => {
                navigate("/dashboard");
            }, 1800);
        },
        onError: (e: any) => {
            toast({ title: "Transfer failed", description: e?.message || "Failed to create transaction", variant: "destructive" });
        },
    });
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const payload: any = {
            ...
            beneficiaryAccount: values.ibanOrAccount,
            beneficiaryBank: values.bankName,
    ...
    return (
        ...
        <PageLayout>
            <div className="flex items-center justify-between mb-6">
                ...
                <h1 className="text-2xl font-semibold text-gray-900">Wire Transfers</h1>
                ...
                <div className="">
                    ...
                    <Card className="bg-inherit py-5">
                        <CardContent className="space-y-4 divide-y">
                            ...
                            <div className="pt-4">
                                <h3 className="text-banking-primary font-semibold">Italian Secure Wire Transfer</h3>
                                <p className="text-gray-500 text-sm">Transfer Funds to another Bank</p>
                            </div>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="beneficiaryName" className="border-0 shadow-none focus-visible:ring-0 px-0" placeholder="" {...form.register("beneficiaryName")} />
                                </div>
